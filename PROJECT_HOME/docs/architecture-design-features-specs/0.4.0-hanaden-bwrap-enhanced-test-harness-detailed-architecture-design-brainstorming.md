...
### 8.2 Navigation Model — Tab-Based + Sidebar

**Two navigation axes: tabs (content view) and sidebar (scope filter + mode switch).**

```
Sidebar (fixed, left):
  +-- Filter input (keystroke search)
  +-- "All" -> switches to Overview mode
  +-- Feat/Spec tree (collapsible, click to filter)
  +-- Run History list (click to switch to Detail mode)

Tabs (top bar):
  Dashboard | Tests | Coverage | Failures | Trends
```

```mermaid
graph TD
    subgraph "Sidebar Navigation"
        ALL["'All' - Overview Mode"]
        FEAT["Feature nodes - collapsible, show specs"]
        RUNLIST["Run History - click switches to Detail mode"]
    end

    subgraph "Tab Views"
        DASH["Dashboard - charts + run history table"]
        TESTS["Tests - full test table"]
        COV["Coverage - per-feature pass-rate table"]
        FAIL["Failures - expandable error output"]
        TRENDS["Trends - 3 SVG trend charts"]
    end

    subgraph "Dashboard Modes"
        OV["Overview - aggregate across runs"]
        DT["Detail - single run drill-down"]
    end

    ALL --> OV
    RUNLIST -->|"click run"| DT
    FEAT -->|"click feature"| TESTS
    DASH --> OV
    DASH --> DT
```

### 8.3 Dashboard — Overview Mode (no `--primary`)

When no `--primary` is specified, or when the user clicks "All" in the sidebar:

- **Summary bar** (fixed top): project name | mode label | tests | pass | fail | skip | cov% | time
- **Mode label**: "Overview"
- **4 chart grid** (2x2):
  - **Pass Rate Trend** — SVG line chart across all discovered runs
  - **Test Count & Duration** — bar + line combined chart
  - **Failures Over Time** — red area chart
  - **Latest Run Summary** — donut chart (pass rate %) from latest run's JUnit data
- **Run History table**: sortable table of all discovered runs with Run|Tests|Pass|Fail|Duration columns. Rows are clickable -> switch to Detail mode for that run.

### 8.4 Dashboard — Detail Mode (`--primary NAME` or click run)

When `--primary` is specified, or when a run is clicked in the sidebar/run-history table:

- **Mode label**: run name (truncated to 25 chars)
- **4 chart grid** (2x2):
  - **Test Results** — donut chart
  - **Timing by Feature** — horizontal bar chart (top features by duration)
  - **Coverage by Feature** — horizontal bar chart (pass-rate or JaCoCo coverage per feature)
  - **Trend: Pass Rate** — sparkline across runs

### 8.5 Tabs

| Tab | Content |
|:---|:---|
| **Dashboard** | Overview or Detail chart grid (see 8.3, 8.4) |
| **Tests** | Full sortable test table: St, Test, Feature, Spec, Time |
| **Coverage** | Per-feature pass-rate table with progress bars: Feature, Pass/Total, Coverage, Failures, Specs. When JaCoCo XML is present, shows JaCoCo package data instead. |
| **Failures** | Expandable details for each failed test with error output |
| **Trends** | 3 SVG trend charts: Pass Rate, Test Count & Duration, Failures Over Time |

### 8.6 Sidebar

**Fixed left panel** (220px wide):

1. **Filter input** — keystroke search filters feat/spec tree
2. **"All" node** — click returns to Overview mode
3. **Feat/Spec tree** — collapsible feature nodes showing specs, with test count + fail count badges
4. **Run History section** — list of all discovered runs with timestamp + test count, clickable to switch to Detail mode

### 8.7 Coverage Fallback — Pass-Rate as Proxy

When no JaCoCo XML exists (bats tests do not produce JaCoCo), the coverage tab and dashboard chart use **test pass-rate per feature** as a coverage proxy:

| Has JaCoCo? | Coverage Tab Shows | Dashboard Chart Shows |
|:---|:---|:---|
| Yes | JaCoCo package coverage (methods/lines/classes) | JaCoCo coverage bars by package |
| No | Pass-rate per feature with progress bars | Pass-rate bars per feature |

### 8.8 JUnit XML Discovery

The report generator searches for JUnit XML in the detail run directory in this order:
1. `{run_dir}/junit.xml` (legacy location)
2. `{run_dir}/_bats-junit-raw/report.xml` (bats harness output location)

### 8.9 Data Architecture — `window.__DATA`

All data is embedded in a single `<script type="application/json" id="report-data">` block, parsed on load:

```javascript
window.__DATA = {
  mode: "overview" | "detail",
  primary_run: "dirname" | null,
  project_name: "bwrap-enhanced",
  generated_at: "2026-09-01T14:42:51Z",
  generator: "merged_html_report.py v0.4.0",
  has_junit: true,
  has_jacoco: false,
  tests_total: 839,
  tests_passed: 752,
  ...
  time_ms: 98197,
  coverage_pct: "89.6",
  trend_data: [ /* array of run summaries for charts */ ],
  runs_meta: [ /* array of all discovered runs for sidebar + table */ ],
  suites: [ /* JUnit suite data */ ],
  test_cases: [ /* flat list of all test cases */ ],
  failures: [ /* failed tests with error output */ ],
  packages: [ /* JaCoCo packages (empty if no JaCoCo) */ ]
};
```

### 8.10 JavaScript Architecture
...
(function() {
  ...
  // Tab switching
  initTabs();        // Dashboard | Tests | Coverage | Failures | Trends

  // Sidebar: feat/spec tree filtering
  initSidebarFilter();  // keystroke search -> hide/show tree nodes
  initSidebarTree();    // click feat -> expand/collapse, filter tables

  // Table sorting
  initSorting();     // column click -> sort ascending/descending

  // Overview/Detail mode switching
  initModeSwitch();  // run-history-row click -> showDetail()
                     // "All" click -> showOverview()
})();
```

---

## 9. Design System — Hanaden Dark v2 (High Contrast)

| Token | Value | Usage |
|:---|:---|:---|
| `--bg-0` | `#0a0a14` | Page background |
| `--bg-1` | `#0f0f1a` | Sidebar, tab bar |
| `--bg-2` | `#1a1a2e` | Cards, summary bar, chart boxes |
| `--bg-3` | `#16213e` | Table headers, active rows |
| `--bg-hover` | `#232342` | Row/item hover state |
| `--border` | `#3a3a5a` | Borders, grid lines |
| `--txt` | `#f8fafc` | Primary text (near-white) |
| `--txt2` | `#e2e8f0` | Secondary text (labels, headers) |
| `--txt3` | `#b0bec5` | Tertiary text (timestamps, captions) |
| `--txt-h` | `#ffffff` | Headings, emphasized text |
| `--accent` | `#b4a0ff` | Active tab, links, mode label |
| `--pass` | `#6ee7a0` | Pass indicators (vivid green) |
| `--fail` | `#ff6b6b` | Fail indicators (vivid red) |
| `--skip` | `#fcd34d` | Skip/warning (vivid amber) |
| `--font-mono` | `'JetBrains Mono', monospace` | Numbers, code, paths |
| `--font-sans` | `'Inter', sans-serif` | UI text |

> **Contrast principle:** All text tokens MUST achieve >= 4.5:1 contrast ratio against their background surface. Status colors (pass/fail/skip) MUST be vivid enough to read at a glance against `--bg-2` and `--bg-3`.
...
# Generate report (overview mode)
bwrap-enhanced-test-harness-runner.sh report html \
    --dir-scan target/test.run.report/ \
    -o target/test.run.report/

# Generate report (detail mode)
bwrap-enhanced-test-harness-runner.sh report html \
    --dir-scan target/test.run.report/ \
    -o target/test.run.report/ \
    --primary 20260901-134612-528175.run-all.test.run

# Verify output
test -f target/test.run.report/site/index.html && echo "OK"
ls -la target/test.run.report/site/index.html
```

### Manual — file:// open (NOT http server)
```bash
xdg-open target/test.run.report/site/index.html
```
- Dashboard renders in Overview mode with 4 chart grid
- All 5 tabs switch correctly (Dashboard, Tests, Coverage, Failures, Trends)
- Sidebar feat/spec tree expands/collapses
- Sidebar filter narrows tree by keystroke
- Run History table rows clickable -> switch to Detail mode
- "All" sidebar click returns to Overview mode
- Coverage tab shows per-feature pass-rate bars (not blank)
- SVG charts render (donut, line, area, bar)
