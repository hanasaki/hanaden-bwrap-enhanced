...
| `--output` | DIR | ✗ **required** | Output directory for `index.html` + `report.tar.gz` |
| `--log-level` | NAME\|NUMBER | `INFO` (400) | Filter log verbosity |

**Tool invoked:** `python3 ${_LIB_DIR}/report-generators/merged_html_report.py --dir-scan DIR --maxdepth N --output DIR`

**Examples:**
```bash
# Typical: scan all runs under target/test.run.report/
...
```
python3 merged_html_report.py --dir-scan DIR --output DIR [OPTIONS]
```
...
| `--output DIR` | ✓ | — | Output directory for `index.html` + `report.tar.gz` |
...
```
target/test.run.report/html/
  index.html        ← self-contained SPA (~12.6MB raw for 20 runs)
  report.tar.gz     ← compressed (~1-2MB)
```
