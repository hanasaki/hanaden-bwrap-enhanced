...
> **Version:** 0.2.0 (JSONL-first pipeline + kcov integration + reporting subsystem)
> **Status:** IMPLEMENTED
...
4. **Coverage mapping heuristic.** For fast iteration, the harness maps pass/fail to covered/missed as a JaCoCo-compatible approximation. Real instrumented line-level coverage is provided by `kcov` (see §14). The mapping heuristic is useful for structure validation but is NOT a substitute for instrumented coverage
...
| kcov | 42+ | Instrumented Bash code coverage → Cobertura XML / HTML / JSON |
...
### Pass/Fail-to-Coverage Mapping Heuristic

> [!WARNING]
> This mapping is a **structural approximation**, not instrumented coverage.
> It is useful for JaCoCo-compatible report generation but does NOT measure
> which source lines were executed. For real line-level coverage, use `kcov`
> integration (see §14).
...
| Property | Value |
|:---|:---|
| **Producer** | `jsonl-to-jacoco-xml.sh` (from JSONL, pass/fail mapping heuristic) or `kcov` (instrumented) |
...
| `kcov` integration for Bash coverage | **Planned (Phase 5)** | Wraps bats execution with `kcov --include-path=src/main` to produce real line-level Cobertura XML. Harness gains `--coverage` flag to enable. See implementation plan Phase 5 |
