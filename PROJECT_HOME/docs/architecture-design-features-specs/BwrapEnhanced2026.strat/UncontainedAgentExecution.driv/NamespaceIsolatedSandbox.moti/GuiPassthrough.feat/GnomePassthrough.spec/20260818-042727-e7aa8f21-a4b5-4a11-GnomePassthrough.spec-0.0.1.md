...
description: >
  ...
  RW-bound: gvfs, gvfsd, doc, dconf, keyring, gcr. --enable-gnome also implies
  --enable-dbus (ENABLE_DBUS=true is set by the CLI parser). Without --enable-gnome,
  none of these GNOME service sockets are present in the sandbox.
...
`--enable-gnome` also implies `--enable-dbus` in the CLI parser (L743):
`ENABLE_DBUS="true"` is set automatically when `--enable-gnome` is parsed.
...
AI agents automating GNOME workflows (form filling, file open/save dialogs,
credential-protected services) need these services to function. Without them,
GNOME apps silently degrade: missing themes, broken file pickers, inaccessible secrets.

All sockets are RW-bound because GNOME IPC is bidirectional (client sends requests,
daemon sends responses and signals). `--bind-try` ensures no FATAL on non-GNOME hosts.
...
| ENABLE_DBUS implied | yes (set by CLI parser) | Always with --enable-gnome |
...
**Value:** 9 RW binds (gvfs, gvfsd, doc, dconf, keyring, gcr + 3 --dir) + DBUS implied
...
# DBUS implied (DbusIpcWritable also active):
./bwrap-enhanced.sh ... --enable-gnome -- bash -c \
  'stat /run/user/$(id -u)/bus 2>&1'
# Expected: socket exists (D-Bus also bound)
...
| GN-005 | ENABLE_DBUS implied | `stat $RU/bus` inside | bus socket present | NOT_STARTED |
...
- bwrap-enhanced.sh L743 (CLI: --enable-gnome sets ENABLE_DBUS=true)
...
graph LR
    ...
    A --> C["--bind-try gvfs gvfsd doc"]
    A --> D["--dir dconf\n--bind-try dconf"]
    A --> E["--dir keyring\n--bind-try keyring"]
    A --> F["--dir gcr\n--bind-try gcr"]
    B & C & D & E & F --> G{"GNOME session\nrunning on host?"}
    G -->|"Yes"| H["GNOME apps: full\nservice integration\n✅ Files/secrets/settings"]
