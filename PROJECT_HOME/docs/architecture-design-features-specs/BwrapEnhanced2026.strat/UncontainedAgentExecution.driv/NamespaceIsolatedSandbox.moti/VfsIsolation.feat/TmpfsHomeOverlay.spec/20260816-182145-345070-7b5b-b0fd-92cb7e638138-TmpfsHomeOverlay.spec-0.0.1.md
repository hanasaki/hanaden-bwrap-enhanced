...
description: >
  ...
  all host home directories including autofs-backed NFS homes. Only /home/VUSER
  (the egress bind) is visible under this tmpfs. This prevents any host user's home
  directory from being accessible, even if the host uses NFS autofs for /home.
...
NFS-mounted systems may include hundreds of user homes). This creates two risks:
1. **Privacy**: sandboxed processes can read other users' home directory names and
   potentially their world-readable files
2. **NFS hangs**: accessing autofs-managed `/home/username` triggers NFS mount probes
...
**Rationale:** Any other filesystem type (ext4, nfs, overlay) means the host homes are exposed.
...
# On NFS host — verify no NFS mount triggered:
time ls /home
# Expected: exits < 0.1s (no autofs probe)
```
...
| THO-004 | No NFS hang on ls /home | `time ls /home` inside | < 0.5s | NOT_STARTED |
...
| THO-S002 | NFS home not mounted | `mount \| grep /home` inside | only tmpfs entry | NOT_STARTED |

## References

- bwrap-enhanced.sh L394-L400 (--tmpfs /home in exec_sandbox step 8)
- HostShadowing.feat AutofsShadow.spec (related: /homes shadowing)
