...
description: >
  Feature: Host-specific paths (autofs /homes, /etc/profile, /etc/profile.d) are
  shadowed inside the sandbox to prevent NFS hangs, conda/nvm contamination, and
...
the sandbox: NFS-backed autofs /homes that hang on access, /etc/profile.d full of
...
graph TD
    ...
    F --> AFS[AutofsShadow.spec\n--tmpfs /homes]
    F --> PR[ProfileReplacement.spec\n--ro-bind-data 11 /etc/profile]
    F --> PDS[ProfileDirSuppression.spec\n--dir /etc/profile.d empty]
```
