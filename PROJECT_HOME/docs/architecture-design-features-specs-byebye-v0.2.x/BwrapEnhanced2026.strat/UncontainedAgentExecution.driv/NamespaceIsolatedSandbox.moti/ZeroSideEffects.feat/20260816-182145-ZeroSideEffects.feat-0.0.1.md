...
graph TD
    ...
    F --> ZM["ZeroMutation.spec<br/>no mktemp/write to host"]
    F --> MRF["MissingRootFatal.spec<br/>HOST_ROOT must exist"]
    F --> MHF["MissingHomeFatal.spec<br/>HOST_HOME must exist"]
    F --> ECF["EmptyCommandFatal.spec<br/> CMD must be non-empty"]
```
