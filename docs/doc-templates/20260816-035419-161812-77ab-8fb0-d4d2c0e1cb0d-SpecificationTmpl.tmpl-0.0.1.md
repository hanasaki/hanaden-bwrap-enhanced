...
---
doc-id:      20260816-035419-161812-77ab-8fb0-d4d2c0e1cb0d-SpecificationTmpl
version:      0.0.1
status:       ACTIVE
category:     TEMPLATE
...
```mermaid
flowchart LR
    ARCH["🏗️ SpatialVisionGuidance.arch<br/>Vision Guidance System"]:::arch
...
> [!NOTE]
> **Directory convention:** Specification documents live in `Slug.spec/` directories.
> Even leaf specs (no sub-specs) get their own directory — this enables test co-location.
> No UUIDv7 on directory names.

> [!NOTE]
> **`status` vs `tdd.state` — independent axes:**
> `status: Draft | Review | Accepted | Implemented | Superseded` = **document lifecycle**.
> `tdd.state: NOT_STARTED | RED | GREEN | REFACTOR` = **test lifecycle**.
> A spec can be `Accepted` but still `RED` (approved requirement, no passing test yet).
> The Specification Statement MUST be detailed enough for an AI agent to generate
> a test script from it without ambiguity.
...
| 0.0.1 | 2026-08-16 | Frederick Bloom | Schema refactor: drop `node-id`/`children`, `unlocks` uses `filename-id`, add definition block with examples |
| 0.0.1 | 2026-08-16 | Frederick Bloom + AI | Add tdd: frontmatter, TDD column, test-file, no-ordering note, status vs tdd, Slug.class naming, dir convention |
