...
```mermaid
flowchart BT
    SPEC_NNN["SPEC-NNN<br/>Spec Title"]:::specStyle
    FEAT_SUB["FEAT-SubA<br/>Sub-Feature Title"]:::featStyle
    ARCH_NNN["ARCH-NNN<br/>Architecture Title"]:::archStyle
...
### Feature F1: [Sub-Feature Slug] — [Title]

[1 sentence description]

#### Spec S1.1: [Spec Slug] — [Title]
...
> Repeat the `### Feature F{N}` block for each sub-feature.
...
| 0.0.2 | 2026-08-16 | Frederick Bloom | Schema refactor: drop `node-id`/`children`/`unlocked-by`, all pointers use `filename-id`, add definition block |
