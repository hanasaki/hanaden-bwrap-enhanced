...
flowchart LR
    ...
    CS(["👑 DomFulfillmentAutomation2026.strat\nDominate fulfillment automation\n— cut delivery times in half"]):::corp

    %% Layer 1 — Business
    BD(["💼 EcommTransitFatigue.driv\nE-commerce loses $2B/yr\nin worker transit fatigue"]):::driv
    BM(["💼 IndoorDroneFleet.moti\nDeploy indoor drone fleet\nto move bins under 45s"]):::moti

    %% Layer 1 — Technical
    TD2(["🛠️ LegacyWifiDrops.driv\nLegacy Wi-Fi drops signals\nin deep metallic aisles"]):::driv
    TM(["🛠️ LocalMeshRouting.moti\nLocal mesh routing to\nbypass Wi-Fi dependency"]):::moti

    %% Layer 2 — Features
    F300(["🚗 AutonomousNavigation.feat\nAutonomous Bin\nTransportation"]):::feat
    F300A(["🚗 AutonomousNavigation.feat-SubA\nPrecision Drone\nDocking"]):::feat
    F300A1(["🚗 AutonomousNavigation.feat-SubA1\nAutomated Battery\nHot-Swapping"]):::feat
    F301(["🚗 BlindSpotNightVisionNav.feat\n🌟 Blind-Spot\nNight-Vision Nav"]):::unlock

    %% Layer 3 — Architectures
    A500(["🏗️ SpatialVisionGuidance.arch\nSpatial Computer Vision\nGuidance System"]):::arch
    A501(["🏗️ UwbDecentralizedMesh.arch\nUWB Decentralized\nMesh Network"]):::arch

    %% Layer 3-4 — Specifications
    SP400(["📐 PayloadCapacity.spec\nPayload Carrying\nCapacity"]):::spec
    SP400A(["📐 PayloadCapacity.spec-SubA\nMax Liftoff\nMass Limit"]):::spec
    SP400B(["📐 PayloadCapacity.spec-SubB\nFrame Deflection\nTolerance"]):::spec
    SP600(["📐 OpticalLidarArray.spec\nOptical LiDAR\nImaging Array"]):::spec
    SP600A(["📐 OpticalLidarArray.spec-SubA\nLaser Emitter\nPulse Frequency"]):::spec
    SP700(["📐 P2pTransceiverNetwork.spec\nP2P Transceiver\nNetwork"]):::spec
    SP700A(["📐 P2pTransceiverNetwork.spec-SubA\nData Packet Transfer\nFrequency"]):::spec
    SP700B(["📐 P2pTransceiverNetwork.spec-SubB\nSignal Encryption\nStandard"]):::spec
...
│
└── 👑 Layer 0: DomFulfillmentAutomation2026.strat | Dominate fulfillment center automation by cutting delivery times in half.
    │   └── 🧪 TEST SUITE: Market Strategy Validation
    │       └── 📊 Perf: Benchmark total logistics transit times across test facility layouts.
    │
    ├── 💼 Layer 1: EcommTransitFatigue.driv | E-commerce facilities lose $2B annually in worker transit fatigue.
    │   │   └── 🧪 TEST SUITE: Macro-Economic Analysis
    │   │       └── 📊 Perf: Monitor and verify facility-wide picker idle times.
    │   │
    │   └── 💼 Layer 1: IndoorDroneFleet.moti | Deploy an indoor drone fleet to move bins under 45 seconds.
    ...
    │       │
    │       └── 🚗 Layer 2: AutonomousNavigation.feat | Autonomous Bin Transportation Feature
    ...
    │           │   └── 🚗 Layer 2: AutonomousNavigation.feat-SubA | Precision Drone Docking Feature
    ...
    │           │       │
    │           │       └── 🚗 Layer 2: AutonomousNavigation.feat-SubA1 | Automated Battery Hot-Swapping Feature
    ...
    │           │   └── 📐 Layer 3: PayloadCapacity.spec | Payload Carrying Capacity Spec
    ...
    │           │       │   └── 📐 Layer 4: PayloadCapacity.spec-SubA | Maximum Liftoff Mass Limit Spec
    ...
    │           │       │
    │           │       └── 📐 Layer 4: PayloadCapacity.spec-SubB | Structural Frame Deflection Tolerance
    ...
    │           │
    │           └── 🏗️ Layer 3: SpatialVisionGuidance.arch | Spatial Computer Vision Guidance System Architecture
    ...
    │               │
    │               └── 📐 Layer 4: OpticalLidarArray.spec | Optical LiDAR Imaging Array Spec
    ...
    │                   │
    │                   ├── 📐 [CASE: SPEC HAS SPEC (SUBSPEC)]
    │                   │   └── 📐 Layer 4: OpticalLidarArray.spec-SubA | Laser Emitter Pulse Frequency Spec
    ...
    │                       └── 🚗 Layer 2: BlindSpotNightVisionNav.feat | "Blind-Spot" Night-Vision Navigation Feature
    ...
    │
    └── 🛠️ Layer 1: LegacyWifiDrops.driv | Legacy facility Wi-Fi drops signals in deep metallic storage aisles.
        │   └── 🧪 TEST SUITE: RF Field Analysis
        │       └── 📊 Perf: Record wireless packet drop ratios across long warehouse storage aisles.
        │
        └── 🛠️ Layer 1: LocalMeshRouting.moti | Implement local mesh data routing to bypass Wi-Fi dependency.
            ...
            │
            └── 🏗️ Layer 3: UwbDecentralizedMesh.arch | Ultra-Wideband (UWB) Decentralized Mesh Network Architecture
                ...
                │
                └── 📐 Layer 4: P2pTransceiverNetwork.spec | Peer-to-Peer Transceiver Network Architecture Spec
                    ...
                    │
                    ├── 📐 Layer 4: P2pTransceiverNetwork.spec-SubA | Data Packet Transfer Frequency
                    │   │   └── 🧪 TEST SUITE: Spectrum Analyzer Validation
                    │   │       └── ⚙️  Functional: Verify center frequency limits and channel bleeding constraints.
                    │   └── Value: "6.5 GHz"
                    │
                    └── 📐 Layer 4: P2pTransceiverNetwork.spec-SubB | Signal Encryption Standard
...
2. **Child→parent pointer only** — every non-root node has a `parent` field pointing to its parent's `filename-id`. There are no `children`, `children-order`, `depends-on`, or `order` fields. To find a node's children: `ls *.{class}/` in that node's directory.
...
10. **Directory = parent chain** — filesystem directories use `Slug.class/` naming. The path from root to leaf traces the `parent:` chain. `cd ..` walks up one parent pointer.
